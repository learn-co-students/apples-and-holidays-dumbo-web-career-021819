...
def all_winter_holiday_supplies(holiday_hash)
  ...
  array_of_supplies = holiday_hash[:winter].map do |holiday, items|
     items
  end
...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each{|season, holiday|
    ...
    puts season.to_s.capitalize! << ":"  #formats "New years:"
    
    holiday.each {|holiday, supplies|
      
      supplies = supplies.join(", ") #formats to "Lights, Wreath"
      holiday = holiday.to_s.split("_").map {|x| x.capitalize}.join(" ") << ":"  
      
      # new_years -> "new years" -> ["New", "Years"] -> "New Years" -> "New Years: "
